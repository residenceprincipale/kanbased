...
export function useColumnsSuspenseQuery(boardName: string) {
  ...
  return useSuspenseQuery({
    ...queryOptions, select: transformColumnsQuery,
  });
}
...
export function transformColumnsQuery(data: ColumnsQueryResponse) {
  ...
  return { boardId: data.boardId, boardName: data.boardName, columns: Array.from(columnWithTasksMap.values()) }
}
...
export function useCreateColumnMutation(boardName: string) {
  ...
  return api.useMutation("post", "/columns", {
    onMutate: async (variables) => {
      ...
      queryClient.setQueryData(queryKey, (oldData: ColumnsQueryResponse): ColumnsQueryResponse => {
        return {
          ...oldData,
